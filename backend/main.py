...
# NOTE: this MongoClient is created at import time, before any process fork.
# If this app is ever run with multiple workers (e.g. `uvicorn --workers N` or
# a preforking server), each worker must create its own client *after* the
# fork - a client shared across forked processes is a known pymongo hazard.
...
def _load_collection(collection) -> dict:
    """Load a whole collection into a dict keyed by its natural id (_id)."""
    return {doc["_id"]: {k: v for k, v in doc.items() if k != "_id"} for doc in collection.find()}


def _sync_collection(collection, data: dict) -> None:
    """Make a collection's contents match `data` exactly: upsert every entry,
    then delete any document whose key is no longer present in `data`."""
    ops = [ReplaceOne({"_id": key}, {**value, "_id": key}, upsert=True) for key, value in data.items()]
    if ops:
        collection.bulk_write(ops)
    existing_ids = set(collection.distinct("_id"))
    to_delete = existing_ids - set(data.keys())
    if to_delete:
        collection.delete_many({"_id": {"$in": list(to_delete)}})
...
def save_users_db(users_data):
    """Save users to Mongo"""
    _sync_collection(users_col, users_data)

def load_sessions_db():
    """Load non-expired sessions from Mongo"""
    data = _load_collection(sessions_col)
...
def save_sessions_db(sessions_data):
    """Save sessions to Mongo (datetimes are stored natively as BSON dates)"""
    _sync_collection(sessions_col, sessions_data)

def load_clients_db():
    """Load clients from Mongo"""
    return _load_collection(clients_col)
...
def save_campaigns_db(campaigns_data):
    ...
    _sync_collection(campaigns_col, campaigns_data)

def load_campaign_results_db():
    """Load campaign results from Mongo"""
    return _load_collection(campaign_results_col)

def save_campaign_results_db(results_data):
    """Save campaign results to Mongo"""
    _sync_collection(campaign_results_col, results_data)
...
def get_call_prompt(city_name: str = "",
    ...
    spoken_email_or_fallback = spoken_email if spoken_email else "our office email"
...
{email_line}
    ...
    **IMPORTANT:** Deliver this opening line slowly and very clearly, with a distinct pause between the two parts. Wait for the patient's response before proceeding.
    ...
    • IF CONFIRM:
    Say: "Excellent! We are glad to have you. Just a reminder to please arrive 15 minutes early for check-in. If you have any insurance changes, please email them to {spoken_email_or_fallback}. For any other issues, you can call the office at {clinic_phone}. Do you have any questions?"
    Wait for response.
...
# office_location values in the uploaded sheet are formatted as a long descriptive
# key (e.g. "... <address/provider words> ... <city name>") with the city name always
# starting at the 21st space-separated word (index 20). Shorter keys are used as-is.
CITY_NAME_WORD_INDEX = 20


def extract_city_name(office_location_key: str) -> str:
    """Extract the spoken city name from an office_location key."""
    words = office_location_key.split(" ") if office_location_key else []
    if len(words) > CITY_NAME_WORD_INDEX:
        return " ".join(words[CITY_NAME_WORD_INDEX:])
    return office_location_key
...
async def make_call(call_request: CallRequest, country_code: str = "+1", client_id: Optional[str] = None):
    ...
    try:
        ...
        # Fetch client voice/branding preference if a client_id was supplied
        client_voice = None
        ...
        if client_id and client_id in clients_db:
            ...
            clinic_name = client.get("name", clinic_name)
...
async def delete_client(request: Request, client_id: str):
    ...
    # Delete associated campaign results. Locked because campaign_results_db is
    # also written by background campaign runs, CSV uploads, and the Bland
    # webhook - holding the lock avoids two full-collection syncs racing.
    async with campaign_results_lock:
        results_to_delete = [
            result_key for campaign_id in campaigns_to_delete
...
async def _store_campaign_run_result(campaign_id: str, campaign_name: str, client_name: str,
                                     status: str, results: list, error: Optional[str] = None):
    """Persist a campaign run's outcome (completed or failed) under a lock to avoid
    concurrent runs of the same campaign computing the same run_number."""
    async with campaign_results_lock:
        ...
        if error:
            campaign_results["error"] = error

        campaign_results_db[campaign_run_id] = campaign_results
        await asyncio.to_thread(save_campaign_results_db, campaign_results_db)
...
async def _run_campaign_background(campaign_id: str, campaign: dict, client: dict,
    ...
    try:
        ...
        print(f"🚀 [BG] Starting campaign '{campaign_name}' — {len(call_requests)} calls, max_attempts={max_attempts}")
        ...
        import traceback
        print(f"❌ [BG] Campaign '{campaign_name}' background error: {e}")
        traceback.print_exc()
        await _store_campaign_run_result(
...
async def process_calls_with_retry_and_batching(call_requests, api_key, max_attempts, retry_interval_minutes, campaign_name, campaign_id, client_voice: Optional[str] = None,
    ...
    if exhausted_calls:
        ...
        for call_data in exhausted_calls:
            try:
                await send_final_voicemail(call_data['call_request'], api_key, client_voice, clinic_name=clinic_name)
    ...
    try:
        ...
        if call_requests:
            ...
            for call_request in call_requests:
                try:
                    result = await make_single_call_async(call_request, api_key, semaphore, csv_session_id, client_voice,
                                                          clinic_name=clinic_name, clinic_email=clinic_email, clinic_website=clinic_website)
...
def get_voicemail_prompt(patient_name: str = "[patient name]",
                         ...
                         clinic_phone: str = "[clinic phone]",
                         clinic_name: str = "the clinic") -> str:
    """Get the voicemail message prompt"""

    return f"""
    ROLE & PERSONA
    You are an AI voice agent leaving a voicemail message from {clinic_name}. You are professional, clear, and concise.

    VOICEMAIL MESSAGE
    Hi Good Morning, I am calling from {clinic_name}. This call is for {patient_name} to remind him/her of an upcoming appointment on {appointment_date} at {appointment_time} with {provider_name} at {office_location}. Please make sure to arrive 15 minutes prior to your appointment. Also, Please make sure to email us your insurance information ASAP so that we can get it verified and avoid any delays on the day of your appointment. If you wish to cancel or reschedule your appointment, please inform us at least 24 hours in advance to avoid cancellation charge of $25.00. For more information, you can call us back on {clinic_phone}. Thank you and have a blessed day.
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                print(f"📊 Bland AI API response status: {response_status}")
                ...
                if response_status == 200:
                    ...
                    error_detail = f"API error: Status {response_status}, Response: {await response.text()}"
                    if stored_call_data:
                        ...
                        raise HTTPException(status_code=response_status, detail=error_detail)
...
async def _fetch_call_duration(session: aiohttp.ClientSession, api_key: str, semaphore: asyncio.Semaphore, result: dict) -> None:
    """Fetch a single call's duration from Bland AI and cache it onto the result dict in place."""
    async with semaphore:
        try:
            async with session.get(
                ...
                if response.status != 200:
                    print(f"📊 Dashboard: API error {response.status} for call {result.get('call_id')}")
                    return
                call_data = await response.json()
            ...
            if duration > 0:
                # Cache onto the shared dict so this call never needs re-fetching again.
                result['duration'] = duration
                print(f"📊 Dashboard: Cached fresh {duration}s from {result.get('patient_name', 'Unknown')}")
        except Exception as e:
...
async def get_dashboard_metrics():
    ...
    try:
        ...
        # Build the per-campaign result lists first, and collect every result that's
        # missing a cached duration so they can all be fetched concurrently in one pass
        # (instead of one blocking, sequential API call per result).
        campaign_result_lists = {}
        ...
        for campaign_id in unique_campaign_ids:
            ...
            for stored_key, stored_results in campaign_results_db.items():
                if stored_key == campaign_id or stored_key.startswith(f"{campaign_id}_run_"):
                    run_results = stored_results.get('results', [])
                    all_results.extend(run_results)
                    print(f"📊 Dashboard: Found run {stored_key} with {len(run_results)} calls")

            if all_results:
                campaign_result_lists[campaign_id] = all_results
                total_calls += len(all_results)
        ...
        # Fetch all missing durations concurrently (bounded concurrency so we don't
        # hammer the Bland AI API), instead of one-at-a-time blocking requests.
        if results_needing_fetch:
            api_key = get_api_key()
            if api_key:
                semaphore = asyncio.Semaphore(8)
                async with aiohttp.ClientSession() as session:
                    await asyncio.gather(*[
                        _fetch_call_duration(session, api_key, semaphore, result)
                        for result in results_needing_fetch
                    ])

                # Persist any newly-cached durations so future loads don't re-fetch them.
                async with campaign_results_lock:
                    await asyncio.to_thread(save_campaign_results_db, campaign_results_db)

        # Now sum up durations (stored + whatever was just freshly cached above)
        for campaign_id, all_results in campaign_result_lists.items():
            campaign_duration = 0
            for result in all_results:
                if result.get('success') and result.get('call_id'):
